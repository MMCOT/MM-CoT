...
class APIKeyManager:
    def __init__(self, key_file: str):
        ...
        self.client = Together(api_key=self.keys[self.index])

    def switch(self):
        self.index -= 1
        if self.index < 0:
            raise RuntimeError("All API keys exhausted.")
        print(f"\033[93m[INFO] Switching to new API key (index {self.index})\033[0m")
        self.client = Together(api_key=self.keys[self.index])


key_manager = APIKeyManager("./api_key.txt")
    ...
    while True:
        try:
            ...
            response = key_manager.client.chat.completions.create(
                model=MODEL_NAME, messages=messages, stream=False, temperature=0
            )
            return clean_response_text(response.choices[0].message.content.strip())
        except Exception as e:
            msg = str(e)
            if "402" in msg and "Credit limit exceeded" in msg or "403" in msg:
                key_manager.switch()
            else:
                raise
